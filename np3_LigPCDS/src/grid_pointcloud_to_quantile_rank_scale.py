...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 4:
        ...
        sys.exit("Wrong number of arguments. Three parameters must be supplied in order to read the ligands "
                 "mtz file, xyz file and output path to create the final representations of the ligands in quantile rank scale "
                 "and store to 3D point clouds. The output folder will be a LigPCDS dataset.\nList of parameters: \n"
                 "  1. xyz_labels_path: The path to the data folder called 'xyz_<valid ligand list file name>_<SP|AtomSymbol>' "
                 "where the ligands .xyz files with their labels are located (result of step 3.1.1). "
                 "It must also contain the CSV file with the valid ligands list that had the ligand grid image successfully created. "
                 "This file is named as '<valid ligand list file name>_<SP|AtomSymbol>_box_pc.csv' and is "
                 ...
                 "  3. output_ligPCDS_path: The path to the output folder where the 3D point clouds of the final representations of "
                 "the ligands in quantile rank scale will be stored (i.e. 'data/lig_pcds');\n"
                 ...
                 "  6. row_start: (optional) The number of the row of the '<valid ligand list file name>_<SP|AtomSymbol>_box_pc.csv' "
                 "file where the script should start. Skip to the given row or, if missing, start from the beginning;\n"
                 "  7. row_end: (optional) The number of the row of the '<valid ligand list file name>_<SP|AtomSymbol>_box_pc.csv' "
                 "file where the script should stop. Stop in the given row or, if missing, equal to zero or greater than the number of rows, stop in the last row.\n"
                 )
