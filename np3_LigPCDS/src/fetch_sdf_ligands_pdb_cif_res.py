...
def fetch_sdf_ligands(db_path, ligs_report_file, n, i_start=0):
    ...
    # create the directory 'ligands' if it does not exist yet
    Path(data_folder / 'ligands').mkdir(parents=True, exist_ok=True)
    # create the CIF parser, QUIET set to True to suppress warnings
    ...
    for i in tqdm(range(i_start,n)):
        ...
        # get the list of already downloaded ligands - check their .cif files
        ligs_present = list(dict.fromkeys([lig_pdb.name.split('_')[1] for lig_pdb in
                                         data_folder.glob('ligands/'+structure_id+'_*.cif')]))
        ...
        if not file_pdb_cif.exists():
            print("No PDB CIF found. Skipping structure.")
            continue
        try:
            structure = parser.get_structure(structure_id, file_pdb_cif)
        except:
            print("ERROR PDB CIF parser")
            continue

        # create the CIF IO to extract the ligands from the structure and save to separated .cif files
        io = MMCIFIO()
        ...
        print("** Start downloading ligands SDF data, extracting .cif and parsing the data **")
        for residue in models[0].get_residues():
            if residue.get_id()[0] in ligs_pdbid_list.LigandIDRes.values or residue.get_id()[0] == 'W':
                ...
                if id[0] == 'W':
                    # water ligs, store all water's residue in a single cif file with all the water coordinates
                    io.save((data_folder / 'ligands' / str(structure_id + "_W_lig.cif")).as_posix(),
                    ...
                    if not file_sdf.exists() and retrieve_sdf(file_sdf, structure_id, id[2], id[1]):
                        ...
                        if mol_res[0] is None:  # without sanitize in the download to try to fix later
                            ...
                            file_sdf.unlink(missing_ok=True)
                    # if sdf is present - thus minimal valid
                    # save a cif with the ligand residue, containing the ligand coordinates
                    if file_sdf.exists() and not (data_folder / 'ligands' / str(structure_id + "_" + id_name + "_lig.cif")).exists():
                        io.save((data_folder / 'ligands' / str(structure_id + "_" + id_name + "_lig.cif")).as_posix(),
                                ResSelect(residue.get_id(), id[2]))
