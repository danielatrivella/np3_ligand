...
def organize_imgs_np3_blob_outs(entries_list_path, np3_output_path, new_output_path, db_ligxyz_path):
	...
	for i in tqdm(range(entries_list.shape[0]), desc="Copy blobs imgs to new loc"):
		...
		blob_img_path = np3_output_path / entries_list.entryID[i] / "model_blobs" / "blobs_img" / entries_list.blobID[
			i] / \
		...
		if blob_img_path.exists():
			...
			try:
				...
				#print("entrou")
				n_correctly_label += label_blob_imgs(new_blob_img_path,
				                                     db_ligxyz_path / (entries_list.blobID[i] + '_class.xyz'),
	...
	print("Total of", n_correctly_cp, "/", entries_list.shape[0] - n_missing_img,
	      "blobs image's successfully copied!!\n")
...
if __name__ == "__main__":
	...
	if len(sys.argv) >= 5:
		...
		sys.exit(
			"Wrong number of arguments. Four arguments must be supplied to reorganize a NP3 Blob Label result into a LigPCDS"
