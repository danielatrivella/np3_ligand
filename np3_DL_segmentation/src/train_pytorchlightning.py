...
class MinkowskiSegmentationModule(LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        # reset histogram for next epoch train and IoU computation, if not the last epoch or additional testing epoch
    ...
    def on_validation_batch_end(self, outputs, batch, batch_idx):
        ...
        if self.config.num_devices > 1 and isinstance(outputs, list):
            ...
            outs = outputs
        # log metrics
        self.log_metrics_val(outs, on_step=True)
        return outs
    #
    def on_validation_epoch_end(self):
        #print("validation_epoch_end")
        #print(outputs)
        self.log_metrics_val(None, on_epoch=True)
    #
    ...
    def on_test_batch_end(self, outputs,batch, batch_idx):
        ...
        if self.config.num_devices > 1 and isinstance(outputs, list):
            ...
            outs = outputs
        # log metrics
        self.log_metrics_test(outs, on_step=True)
        return outs
    #
    def on_test_epoch_end(self):
        self.log_metrics_test(None, on_epoch=True)
        # print(outs)  # free memory
    #
